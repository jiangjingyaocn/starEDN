...
from models.starnet import starnet_s1, ConvBN
...
class EDN(nn.Module):
    def __init__(self, arch='mobilenetv2', pretrained=True, use_carafe=True,
        ...
        elif 'starnet_s1' in arch:
            enc_channels=[32, 24, 48, 96, 192, 40, 40]
            dec_channels=[16, 24, 32, 40, 40, 40, 40]
        ...
        # if 'starnet_s1' in arch:
        #     self.final_downsampler = ConvBN(192, 160, kernel_size=1, stride=1, padding=0)  # ✅ 只改变通道数
            # self.final_downsampler = ConvBN(192, enc_channels[4], kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, input):
        ...
        # if 'starnet_s1' in self.arch:
        #     x = self.final_downsampler(backbone_features[-1])
        #     backbone_features[-1] = x
        # print("backbone_features[-1],",backbone_features[-1].shape)
